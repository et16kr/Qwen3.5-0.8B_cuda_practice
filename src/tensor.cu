...
Tensor::Tensor(const std::vector<size_t> &shape_, const float *buf_) {
  reshape(shape_);
  std::memcpy(buf, buf_, num_elem() * sizeof(float));
  to_gpu();
}
...
void Tensor::reshape(const std::vector<size_t> &shape_) {
  ...
  for (size_t i = ndim; i < 6; ++i) {
    shape[i] = 1;
  }

  size_t new_numel = num_elem();
  if (buf == nullptr) {
    buf = static_cast<float *>(std::calloc(new_numel, sizeof(float)));
    CHECK_ERROR(buf != nullptr, "Failed to allocate host tensor of %zu elements",
                new_numel);
    CHECK_CUDA(cudaMalloc(reinterpret_cast<void **>(&gpu_buf),
                          new_numel * sizeof(float)));
...
void Tensor::to_gpu() const {
  CHECK_ERROR(buf != nullptr && gpu_buf != nullptr,
              "Tensor buffers must exist before to_gpu()");
  CHECK_CUDA(cudaMemcpy(gpu_buf, buf, num_elem() * sizeof(float),
                        cudaMemcpyHostToDevice));
}

void Tensor::to_cpu() const {
  CHECK_ERROR(buf != nullptr && gpu_buf != nullptr,
              "Tensor buffers must exist before to_cpu()");
  CHECK_CUDA(cudaMemcpy(buf, gpu_buf, num_elem() * sizeof(float),
                        cudaMemcpyDeviceToHost));
}

void Tensor::zero_host() const { std::memset(buf, 0, num_elem() * sizeof(float)); }
...
TokenBatch::TokenBatch(size_t batch_size, size_t seq_len)
  ...
  buf = static_cast<int32_t *>(std::malloc(n_elem * sizeof(int32_t)));
  CHECK_ERROR(buf != nullptr, "Failed to allocate host token buffer");
  lengths = static_cast<int32_t *>(std::malloc(B * sizeof(int32_t)));
  CHECK_ERROR(lengths != nullptr, "Failed to allocate host length buffer");
  CHECK_CUDA(cudaMalloc(reinterpret_cast<void **>(&gpu_buf),
                        n_elem * sizeof(int32_t)));
  CHECK_CUDA(cudaMalloc(reinterpret_cast<void **>(&gpu_lengths),
                        B * sizeof(int32_t)));
}
...
void TokenBatch::to_gpu() const {
  CHECK_CUDA(cudaMemcpy(gpu_buf, buf, n_elem * sizeof(int32_t),
                        cudaMemcpyHostToDevice));
  CHECK_CUDA(cudaMemcpy(gpu_lengths, lengths, B * sizeof(int32_t),
                        cudaMemcpyHostToDevice));
}
...
void TokenBatch::to_cpu() const {
  CHECK_CUDA(cudaMemcpy(buf, gpu_buf, n_elem * sizeof(int32_t),
                        cudaMemcpyDeviceToHost));
  CHECK_CUDA(cudaMemcpy(lengths, gpu_lengths, B * sizeof(int32_t),
                        cudaMemcpyDeviceToHost));
}
