...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Tokenize text requests, run the Qwen3.5 C++ generator, and decode responses."
    )
...
def encode_request(tokenizer, request: str, system_prompt: str, enable_thinking: bool) -> list[int]:
    if getattr(tokenizer, "chat_template", None):
        ...
        if hasattr(encoded, "get"):
            ...
            if isinstance(input_ids, list):
                return input_ids
        raise TypeError(f"unsupported chat template return type: {type(encoded).__name__}")
...
def read_token_batch(path: Path) -> list[list[int]]:
    ...
    for _ in range(batch_size):
        ...
        offset += 4 * max_len
        sequences.append(row[: lengths[len(sequences)]])
    return sequences
...
def main() -> int:
    ...
    sequences = [
        encode_request(tokenizer, request, args.system, args.enable_thinking)
    ...
    with tempfile.TemporaryDirectory(prefix="qwen3_5_practice_") as tmpdir:
        ...
        if args.keep_temp:
            ...
            (keep_dir / "prompt_tokens.bin").write_bytes(prompt_tokens.read_bytes())
            (keep_dir / "generated_tokens.bin").write_bytes(output_tokens.read_bytes())
